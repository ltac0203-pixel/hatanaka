...
PHP 拡張は Laravel 13 既定：`mbstring`・`pdo_mysql`・`bcmath`・`intl`・`openssl`・`tokenizer`・`xml`・`ctype`・`json`・`fileinfo`・`dom`・`curl`。
...
DB の起動方法は **Docker（推奨）** と **ホスト直接インストール** の 2 ルートを用意しています。どちらか片方を選べば OK。`composer dev` 以降の手順は共通です。

### ルート A: Docker MySQL（推奨）

リポジトリ直下の `docker-compose.yml` で `mysql:8.0`（ホスト側 **3307**）と `mailpit`（SMTP `:1025` / UI `:8025`）が起動します。ホスト側 3306 を別 MySQL が使っていても衝突しないように 3307 を採用しています。

```bash
docker compose up -d
```

| サービス | ホスト側ポート | 既定の認証情報 |
| --- | --- | --- |
| MySQL (`hatanaka-db`) | `127.0.0.1:3307` | DB `hatanaka` / ユーザー `hatanaka` / パスワード `hatanaka`（root は `root`） |
| Mailpit (`hatanaka-mailpit`) | SMTP `127.0.0.1:1025` / UI `127.0.0.1:8025` | 認証なし（`MP_SMTP_AUTH_ACCEPT_ANY=1`） |

テスト DB（`phpunit.xml` の `DB_DATABASE=hatanaka_testing` 固定）はコンテナ内で別途作成：
...
データを完全リセットしたいときは `docker compose down -v`（永続ボリューム `hatanaka_db` も破棄）。

### ルート B: ホスト直接インストール（MySQL 8.0+ または MariaDB 10.6+）
...
# --- DB（ルート B: ホスト直接インストールの場合は上の 5 行を以下で置換） ---
# DB_PORT=3306
...
> `.env` を書き換えた直後は `php artisan config:clear` を必ず実行する。
...
ルート A（Docker）を使っている場合は `docker compose up -d` で `hatanaka-mailpit` が既に起動しているため追加インストール不要。`.env` の MAIL_* を下記に切り替えるだけで使える。

ルート B（ホスト直接インストール）の場合はホストに Mailpit を入れる：
